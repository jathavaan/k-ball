...
describe("Player Rating Test", () => {
  ...
  it("Should navigate to player profile page, give a rating, and delete it", () => {
    ...
    cy.contains("(Rating 2.25)").should("be.visible"); // The average rating we gave should be visible
    ...
    cy.get('[aria-label="Toggle profile view"]').click(); // Open the profile menu
    cy.contains("View my ratings").click(); // Close and then open "View my ratings" to reload the reviews
    cy.contains("View my ratings").click();

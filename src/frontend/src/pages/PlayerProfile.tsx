...
export const PlayerProfile = () => {
  const playerId = 2890;
  const { playerProfileInfo, isLoading, isError } = usePlayerProfileInfoCard(Number(playerId));
  const { playerStatsTable, isLoading: isStatsLoading, isError: isStatsError } = usePlayerStatsTableData(Number(playerId)) || { playerStatsTable: [] };
  ...
  return (
    <>
      {isLoading || isStatsLoading ? (
        ...
        <ErrorAlert message={"Oops! Something went wrong while fetching the player data or stats"} />
      ) : (
        <>
          {playerProfileInfo && (
            <PlayerProfileInfo
